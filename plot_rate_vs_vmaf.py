...
def main(input_file, output_folder, codec, rate,max_bit_rate):

    file_size_input = os.path.getsize(input_file) / (1024 * 1024)
    print("Input file size: ", round(file_size_input, 2), "MB")

    output_file = os.path.join(output_folder, f"output_{file_number}_2_{codec.lower()}_cq{rate}.mp4")
    results, encoding_time = encode_video(input_file, output_file,codec,rate,max_bit_rate)
...
if __name__ == "__main__":
    ...
    for rate in rates:
        ...
        for max_bit_rate in max_bit_rates:
            vmafscore, output_file_size = main(input_file, output_folder, codec, rate,max_bit_rate)
            results.append({
                'rate': rate,
